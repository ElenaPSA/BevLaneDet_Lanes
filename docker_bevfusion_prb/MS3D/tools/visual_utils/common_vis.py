...
def convert_to_o3dbox(boxes, colour=None):
    box_corners = boxes_to_corners_3d(boxes)
    o3d_boxes = []
    for box_corner in box_corners:
        box_corner = o3d.utility.Vector3dVector(box_corner)
        o3dbox = o3d.geometry.OrientedBoundingBox().create_from_points(box_corner)
        if colour is None:
            colour = [1,0,0]
