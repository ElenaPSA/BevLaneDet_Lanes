    ...
    for idx in range(start_idx, start_idx + step):    
       ...
       # box_pts=common_vis.boxes_to_corners_3d(boxes)
        ...
        if len(final_boxes) == 0:
            skipImage = True
            listframe_rejected.append(str(frame_id))
            continue
        
        for k in range(len(instances.matched)):
            if instances.matched[k] == False and instances.valid[k] ==True:
                nb_instances_non_matched+=1
            if instances.valid[k] ==True:
                nb_instances+=1
        ...
        nb_boxes_final+=final_boxes.shape[0]
        listframe_keeped.append(frame_id)

    print("Finish Iteration ", i)
    results_stats={}
    results_stats['nb_boxes']=nb_boxes
    results_stats['nb_boxes_final']=nb_boxes_final
    results_stats['nb_instances']=nb_instances
    results_stats['nb_instances_non_matched']=nb_instances_non_matched
    results_stats['frameids_rejected']=listframe_rejected
    return count, results_dict,results_stats
...
if __name__ == "__main__":
    ...
    for i in range(len(results)):
        ...
        keeped+=results[i][0]
        nb_boxes+=results[i][2]['nb_boxes']
        nb_boxes_final+=results[i][2]['nb_boxes_final']
        nb_instances+=results[i][2]['nb_instances']
        nb_instances_non_matched+=results[i][2]['nb_instances_non_matched']
        rejected_ids.extend(results[i][2]['frameids_rejected'])
    ...
    results_stats['nb_instances']=nb_instances
    results_stats['nb_instances_non_matched']=nb_instances_non_matched
    results_stats['nb_frames_ori']=len(dataset.infos)
    results_stats['nb_frames_keeped']=keeped
    results_stats['percent_keeped']=100*keeped/len(dataset.infos)
    results_stats['frameids_rejected']=rejected_ids
    print(results_stats)
    outpath=str(Path(ms3d_configs["SAVE_DIR"]))+"/results_stats.json"
    with open(outpath, 'w') as f:
        json.dump(results_stats, f)
