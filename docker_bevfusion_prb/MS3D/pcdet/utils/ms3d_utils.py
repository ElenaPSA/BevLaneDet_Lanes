...
# ms3d key functions
from pcdet.utils.compatibility_utils import (get_frame_id, get_lidar, get_pose,
                                             get_sample_idx, get_sequence_name)
from pcdet.utils.tracker_utils import (delete_tracks, get_frame_track_boxes,
                                       get_motion_state)
...
def update_ps(dataset, ps_dict, tracks_veh_all, tracks_veh_static, tracks_bic_all, tracks_bic_static,tracks_ped=None, 
    ...
    for idx, (frame_id, ps) in enumerate(tqdm(final_ps_dict.items(), total=len(final_ps_dict.keys()), desc='update_ps')):        
        ...
        ego_trackall_boxes = np.insert(ego_trackall_boxes[:,:9],7,1,1)
        ego_trackall_boxes[:,9][np.where(ego_trackall_boxes[:,9] < veh_pos_th)[0]] = veh_pos_th
        ...
        ego_static_boxes = np.insert(ego_static_boxes[:,:9],7,1,1)
        ...
        if new_veh_boxes.shape[0] > 1:            
            nms_veh_mask = nms(new_veh_boxes[:,:7].astype(np.float32), 
                            new_veh_boxes[:,9].astype(np.float32),
                            thresh=veh_nms_th)
            new_veh_boxes = new_veh_boxes[nms_veh_mask]

        ## ----- Bicycle -----
        bic_mask = abs(cur_gt_boxes[:,7]) == 3
        cur_bic_boxes = cur_gt_boxes[bic_mask]
        
        # Add dynamic interpolated/extrapolated tracks to replace lower scoring dets
        trackall_bic_boxes = get_frame_track_boxes(tracks_bic_all, frame_id, frame2box_key=frame2box_key)
        pose = get_pose(dataset, frame_id)
        _, ego_trackall_bic_boxes = world_to_ego(pose, boxes=trackall_bic_boxes)
              
        ego_trackall_bic_boxes = np.insert(ego_trackall_bic_boxes[:,:9],7,3,1)
        ego_trackall_bic_boxes[:,9][np.where(ego_trackall_bic_boxes[:,9] < veh_pos_th)[0]] = veh_pos_th
        
        # Add static objects
        trackstatic_bic_boxes = get_frame_track_boxes(tracks_bic_static, frame_id, frame2box_key=frame2box_key_static)
        static_track_ids = np.array([trk_id for trk_id in trackstatic_bic_boxes[:,-1] if tracks_bic_static[trk_id]['motion_state'] == 0])
        static_mask = np.isin(trackstatic_bic_boxes[:,-1], static_track_ids)
        static_bic_boxes = trackstatic_bic_boxes[static_mask]
        _, ego_static_bic_boxes = world_to_ego(pose, boxes=static_bic_boxes)
        ego_static_bic_boxes = np.insert(ego_static_bic_boxes[:,:9],7,3,1)
        
        new_bic_boxes = np.vstack([cur_bic_boxes, ego_trackall_bic_boxes, ego_static_bic_boxes])  
        #new_bic_boxes = np.vstack([ego_trackall_bic_boxes, ego_static_bic_boxes])        
        if new_bic_boxes.shape[0] > 1:            
            nms_bic_mask = nms(new_bic_boxes[:,:7].astype(np.float32), 
                            new_bic_boxes[:,9].astype(np.float32),
                            thresh=veh_nms_th)
            new_bic_boxes = new_bic_boxes[nms_bic_mask]
        ...
        if tracks_ped is not None:
            ...
            if cur_ped_boxes.shape[0] != 0:
                track_boxes_ped = get_frame_track_boxes(tracks_ped, frame_id) # (N,10): [x,y,z,dx,dy,dz,heading,ori_class,score,trk_id]
                pose = get_pose(dataset, frame_id)
                _, ego_track_boxes_ped = world_to_ego(pose, boxes=track_boxes_ped)
                ego_track_boxes_ped = np.insert(ego_track_boxes_ped[:,:9], 7, axis=1, values=2) # (N,10): [x,y,z,dx,dy,dz,heading,cls_id,ori_class_id,score] ; ped class_id = 2        
               # new_ped_boxes = np.vstack([cur_ped_boxes, ego_track_boxes_ped])   
                new_ped_boxes = np.vstack([ego_track_boxes_ped])        
                if new_ped_boxes.shape[0] > 1:            
                    nms_mask = nms(new_ped_boxes[:,:7].astype(np.float32), 
                                    new_ped_boxes[:,9].astype(np.float32),
                                    thresh=ped_nms_th)
                    new_ped_boxes = new_ped_boxes[nms_mask]
            else:
                new_ped_boxes = np.empty((0,10))

            new_boxes = np.vstack([new_veh_boxes, new_bic_boxes, new_ped_boxes])        
        else:
            new_boxes =  np.vstack([new_veh_boxes,new_bic_boxes])
        ...
        for box in new_boxes:
            ...
            num_pts.append(10) #use this for now, since we use fusion
...
def refine_veh_labels(dataset, frame_ids,
                      ...
                      refine_cfg, save_dir=None,cls_id=1):
    """
    ...
    for trk_id in tracks_veh_all.keys():
        score_mask = tracks_veh_all[trk_id]['boxes'][:,8] > veh_pos_th
    ...
    for trk_id in tracks_veh_static.keys():
        score_mask = tracks_veh_static[trk_id]['boxes'][:,8] > static_trk_score_th
    ...
    get_track_rolling_kde_interpolation(dataset, tracks_veh_static, window=refine_cfg['ROLLING_KBF']['ROLLING_KDE_WINDOW'], 
                                                              static_score_th=static_trk_score_th, kdebox_min_score=refine_cfg['ROLLING_KBF']['MIN_STATIC_SCORE'], cls_id=cls_id)  # 16MIN for 18840
...
def refine_ped_labels(tracks_ped, ped_pos_th, track_filtering_cfg, s2e_th=2):
    ...
    for trk_id in all_ids:
        mask = tracks_ped[trk_id]['boxes'][:,8] < ped_pos_th
        tracks_ped[trk_id]['boxes'][:,8][mask] = ped_pos_th
...
def select_ps_by_th(ps_dict, pos_th):
    ...
    for frame_id in ps_dict.keys():
        ...
        veh_mask = np.logical_and(abs(ps_dict[frame_id]['gt_boxes'][:,7]) == 1, 
                                ps_dict[frame_id]['gt_boxes'][:,9] < pos_th[0])
        ...
        ped_mask = np.logical_and(abs(ps_dict[frame_id]['gt_boxes'][:,7]) == 2, 
                                ps_dict[frame_id]['gt_boxes'][:,9] < pos_th[1])
        ...
        cyc_mask = np.logical_and(abs(ps_dict[frame_id]['gt_boxes'][:,7]) == 3, 
                                 ps_dict[frame_id]['gt_boxes'][:,9] < pos_th[2])
...
def get_track_rolling_kde_interpolation(dataset, tracks_static, window, static_score_th, kdebox_min_score,cls_id):
    """
    ...
    for trk_id in tqdm(tracks_static.keys(), total=len(tracks_static), desc='rolling_kde_interp'):
        ...
        if len(trk_frame_inds_set) < window:
            ...
            boxes = np.insert(boxes, 7,cls_id,1) 
           
            kdebox = kbf(boxes, box_weights=boxes[:,-1], bw_score=1.0)
            if kdebox[9] > static_score_th:
                kdebox[9] = max(kdebox_min_score, kdebox[9])
        ...
        for ind in frame_inds:
            ...
            if frame_id in tracks_static[trk_id]['frameid_to_propboxes'].keys():
                ...
                if ind < first_frame_idx:    
                    ...
                    new_score = tracks_static[trk_id]['frameid_to_propboxes'][frame_id][8]
                    new_score = np.clip(new_score * degrade_factor**abs(first_frame_idx-ind), min_score_clip, 1.0)
                    tracks_static[trk_id]['frameid_to_propboxes'][frame_id][8] = new_score
