...
def get_detection_sets(det_annos, score_th=0.1):
    ...
    for idx in tqdm(range(len_data), total=len_data, desc='load detection sets'):
        frame_dets = {}
        frame_dets['boxes_lidar'], frame_dets['score'], frame_dets['source'], frame_dets['source_id'], frame_dets['frame_id'], frame_dets['class_ids'], frame_dets['names'], frame_dets['box_weights'], frame_dets['ori_class_ids'] = [],[],[],[],[],[],[],[],[]
        for src_id, key in enumerate(src_keys):
            ...
            frame_dets['ori_class_ids'].extend(det_annos[key][idx]['label'][score_mask])
        ...
        frame_dets['ori_class_ids'] = np.array(frame_dets['ori_class_ids']) if len(frame_dets['score']) != 0 else  np.array([])
...
def kbf(boxes, box_weights, bw_c=1.0, bw_dim=2.0, bw_ry=0.1, bw_cls=0.5, bw_score=2.0):
    ...
    Args:
        boxes: (N,10) np array. Boxes for filtering
        box_weights : (list). List of weights for each box

    Returns:
        combined box: (10) np array. A final box with params [x,y,z,dx,dy,dz,ry,class_id,ori_class_id,score]
    ...
    if boxes.shape == 2:             
        max_score_ind = np.argmax(boxes[:,9]*box_weights)
        ...
        new_ori_class = boxes[max_score_ind,8]
        new_score = np.average(boxes[:,9], axis=0, weights=box_weights)
        combined_box = np.hstack([centroid[0], centroid[1], centroid[2], dims[0], dims[1], dims[2], new_heading, new_class, new_ori_class,new_score])
    ...
    cls_ind = get_kde_value(boxes[:,8], box_weights, bw_cls, return_max=False)
    if cls_ind is not None:
        new_ori_class = boxes[:,8][cls_ind]
    else:
        # Return majority class
        unique, counts = np.unique(boxes[:,8], return_counts=True)
        new_ori_class = int(unique[np.argmax(counts)])

    new_score = get_kde_value(boxes[:,9], box_weights, bw_score, return_max=True)
    
    combined_box = np.hstack([new_cxyz[0], new_cxyz[1], new_cxyz[2], new_dx, new_dy, new_dz, new_heading, new_class, new_ori_class,new_score])
    return combined_box


def label_fusion(boxes_lidar, discard, radius, nms_thresh=0.05, use_box_weights=False):
    """
    boxes_lidar (N,10): array of box proposals from src detectors
    ...
    return: 
        fused_boxes (N,10): fused box proposals
        raw_labels (N,10): box proposals that contributed to the combined box
    """
    ...
    for m_box_inds in matched_inds_list:
        ...
        if len(unique) < discard:
            continue
        box_weights = matched_boxes[:,9]*matched_boxes[:,10] if use_box_weights else matched_boxes[:,9]
        combined_box = kbf(matched_boxes[:,:10], box_weights=box_weights)
    ...
    if combined_frame_boxes:
        ...
        nms_mask = nms(fused_boxes[:,:7], fused_boxes[:,9], thresh=nms_thresh)
        fused_boxes = fused_boxes[nms_mask]       
    else:
        fused_boxes = np.empty((0,10)).astype(np.float32)
    ...
    for frame_boxes in tqdm(detection_sets, total=len(detection_sets), desc='get initial ps labels'):
        ...
        for class_name in SUPERCATEGORIES:
            ...
            if cls_boxes.shape[0] == 0:
                continue
            cls_boxes = cls_boxes[cls_boxes[:,10] > 0] # Discard if box weight == 0
            if cls_boxes.shape[0] == 0:
                continue
            score_mask = cls_boxes[:,9] > cls_kbf_config[class_name]['neg_th']
