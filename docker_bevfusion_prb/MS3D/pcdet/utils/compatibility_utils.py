...
def get_pose(dataset, frame_id):
    ...
    if dataset.dataset_cfg.DATASET == 'ONCEDataset':
        ...
        return pose
    elif dataset.dataset_cfg.DATASET in ['StellantisDataset']:
        infos_idx = dataset.frameid_to_idx[frame_id]
        pose = np.eye(4).astype(np.float32)
        pose[:3, :3] = dataset.infos[infos_idx]["ego2global_rotation"]
        pose[:3, 3] = dataset.infos[infos_idx]["ego2global_translation"]
...
def get_lidar(dataset, frame_id):
    ...
    elif dataset.dataset_cfg.DATASET in ['NuScenesDataset','LyftDataset','StellantisDataset']:
...
def get_sequence_name(dataset, frame_id):
    ...
    if dataset.dataset_cfg.DATASET == 'ONCEDataset':
        ...
        return dataset.infos[infos_idx]['sequence']
    elif dataset.dataset_cfg.DATASET in ['WaymoDataset','CustomDataset']:
        return dataset.infos[infos_idx]['point_cloud']['lidar_sequence']
    else:
        raise NotImplementedError
...
def get_timestamp(dataset, frame_id):
    ...
    if dataset.dataset_cfg.DATASET == 'ONCEDataset':
        ...
        return dataset.infos[infos_idx]['timestamp']  * 1e-6
    elif dataset.dataset_cfg.DATASET == 'WaymoDataset':
