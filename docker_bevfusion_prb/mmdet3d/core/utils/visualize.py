...
def visualize_camera(
    ...
    *,
    ...
    if bboxes is not None and len(bboxes) > 0:
        ...
        centers=bboxes.center.numpy()
        yaws = bboxes.yaw.numpy()
        dims=bboxes.dims.numpy()
        num_bboxes = corners.shape[0]
        ...
        for index in range(coords.shape[0]):
            ...
            coord_center= np.mean(coords[index][[0,3,7,4]],axis=0)
            coord_center_back=np.mean(coords[index,[0,4]],axis=0)
            
            cv2.line(
                    canvas,
                    coord_center_back.astype(np.int32),
                    coord_center.astype(np.int32),
                    color or OBJECT_PALETTE[name],
                    thickness,
                    cv2.LINE_AA,
                )
...
def export_results_aimotive(
    ...
    if bboxes is not None and len(bboxes) > 0:
        ...
        for i in range(centers.shape[0]): 
            ...
            box["BoundingBox3D Extent X"]=dims[i,1]
            box["BoundingBox3D Extent Y"]=dims[i,0]
