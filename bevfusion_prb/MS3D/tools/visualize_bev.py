...
def main():
    ...
    gt_boxes=compat.get_gt_boxes(target_set, start_frame_id)[class_mask]
    print(gt_boxes)
    ...
    if args.ps_pkl2 is not None:
        combined_mask = ps_dict2[ps_frame_ids[start_idx]]['gt_boxes'][:,9] >= args.conf_th
        ...
        plot_boxes(ax, ps_dict2[ps_frame_ids[start_idx]]['gt_boxes'][combined_mask], 
                scores=ps_dict2[ps_frame_ids[start_idx]]['gt_boxes'][combined_mask][:,9],
                label='ps labels 2', color=[1,0,0],
                limit_range=limit_range, alpha=1, show_score=False if args.hide_score else True) 
        
    if args.ps_pkl is not None:
        combined_mask = ps_dict[ps_frame_ids[start_idx]]['gt_boxes'][:,9] >= args.conf_th
        if args.above_pos_th:
        ...
        plot_boxes(ax, ps_dict[ps_frame_ids[start_idx]]['gt_boxes'][combined_mask], 
                scores=ps_dict[ps_frame_ids[start_idx]]['gt_boxes'][combined_mask][:,9],
