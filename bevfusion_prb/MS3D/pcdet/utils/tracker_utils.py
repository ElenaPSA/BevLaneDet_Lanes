...
from pcdet.utils.transform_utils import ego_to_world, pose_delta
from tqdm import tqdm


def refine_origin_class(track):

    box_cls_id=[box[7] for box in track['update_boxes']]
    
    unique, count= np.unique(box_cls_id,return_counts=True)
  
    cls_id=unique[np.argmax(count)]

    for box in track['boxes']:
        box[7]=cls_id

def refine_sizes(track):

    box_sizes=track['boxes'][-1][3:6] 

    for box in track['boxes']:
        box[3:6]=box_sizes

def get_tracklets(dataset, ps_dict, configs, cls_id,local_tracking=False):
    """
    ...
    for frame_id in tqdm(ps_dict.keys(), total=len(ps_dict.keys()), desc='generate_trks'):
        ...
        if local_tracking:
            if np.size(prev_pose)!=0:
                delta_pose=pose_delta(prev_pose,pose)
            else:
        ...
        ori_class_ids = np.array(abs(boxes[:,8]))
        ...
        if not local_tracking:
            ...
            dets=list(np.hstack([boxes_global[:,:7], boxes_global[:,9].reshape(-1,1)]))
        else:
            dets=list(np.hstack([boxes[class_ids == cls_id,:7], boxes[class_ids == cls_id,9].reshape(-1,1)]))
            points_global=points
       
        frame_data = FrameData(dets=dets, ego=pose,delta_ego=delta_pose, pc=points_global, det_types=class_ids[class_ids == cls_id], 
                            det_ori_types=ori_class_ids[class_ids == cls_id],
                            aux_info=aux_info, time_stamp=timestamp, input_opd_format=True)
        
      
        results = tracker.frame_mot(frame_data)
     
        track_boxes = [BBox.bbox2array_withclass(trk[0], output_opd_format=True) for trk in results]
        update_boxes = [BBox.bbox2array_withclass(trk[4], output_opd_format=True) for trk in results]
        track_ids = [trk[1] for trk in results]
       
        if local_tracking:
            track_boxes_ego=np.array(track_boxes).copy()
            update_boxes_ego=np.array(update_boxes).copy()
            if len(track_boxes)!=0 : 
                _, track_boxes = ego_to_world(pose, boxes=track_boxes_ego)
            if len(update_boxes)!=0 :
                _, update_boxes = ego_to_world(pose, boxes=update_boxes_ego)
        ...
        for list_idx, track_id in enumerate(track_ids):
            ...
            if track_id not in tracks.keys():
                ...
                tracks[track_id]['update_boxes'] = []
                if local_tracking:
                    tracks[track_id]['boxes_ego'] = []
                    tracks[track_id]['update_boxes_ego'] = []
                    tracks[track_id]['pose'] =[]
                tracks[track_id]['frame_id'] = []
                tracks[track_id]['timestamp']=[]
                tracks[track_id]['delta_pose']=[]
               
            tracks[track_id]['boxes'].append(track_boxes[list_idx])
            tracks[track_id]['update_boxes'].append(update_boxes[list_idx])
            
            if local_tracking:
                tracks[track_id]['boxes_ego'].append(track_boxes_ego[list_idx])
                tracks[track_id]['update_boxes_ego'].append(update_boxes_ego[list_idx])
                tracks[track_id]['pose'].append(pose.copy())

            tracks[track_id]['frame_id'].append(f'{frame_id}')
            tracks[track_id]['timestamp'].append(timestamp)
            tracks[track_id]['delta_pose'].append(delta_pose)
       
    for track_id, track in tracks.items():
        refine_origin_class(track)
       # refine_sizes(track)
...
def delete_tracks(tracks, min_score, num_boxes_abv_score=3):
    ...
    for trk_id in all_ids:    
        ...
        if len(np.argwhere(tracks[trk_id]['boxes'][:,8] > min_score)) < num_boxes_abv_score:
            del tracks[trk_id]      
...
def prepare_track_cfg(ms3d_tracking_cfg,use_imm=False):
    """
    ...
    trk_cfg['running']['motion_imm'] = use_imm
    ...
    trk_cfg['running']['angle_thres']=ms3d_tracking_cfg['RUNNING']['ANGLE_TH']
