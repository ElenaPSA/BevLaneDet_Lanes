...
def mod_angle(theta,thetar):
    
    theta_d1 = modulo(theta - thetar + np.pi, 2 * np.pi) - np.pi
    theta_d2 = modulo(theta+np.pi - thetar + np.pi, 2 * np.pi) - np.pi
    if abs(theta_d1)>abs(theta_d2):
        return thetar + theta_d2
    else:
        return thetar + theta_d1
    
def associate_dets_to_tracks(dets, tracks, mode, asso, angle_thresh=-1.0, dist_threshold=0.9, trk_innovation_matrix=None):
    """ associate the tracks with detections
    """
    if mode == 'bipartite':
        matched_indices, dist_matrix = \
            bipartite_matcher(dets, tracks, asso, dist_threshold,angle_thresh, trk_innovation_matrix)
    elif mode == 'greedy':
        matched_indices, dist_matrix = \
            greedy_matcher(dets, tracks, asso, dist_threshold,angle_thresh, trk_innovation_matrix)
    unmatched_dets = list()
...
def bipartite_matcher(dets, tracks, asso, dist_threshold,angle_thresh, trk_innovation_matrix):
    if asso == 'iou':
        dist_matrix = compute_iou_distance(dets, tracks, asso,angle_thresh)
    if asso == 'iou_2d':
        dist_matrix = compute_iou_distance(dets, tracks, asso,angle_thresh)
    elif asso == 'giou':
        dist_matrix = compute_iou_distance(dets, tracks, asso,angle_thresh)
...
def greedy_matcher(dets, tracks, asso, dist_threshold, angle_thresh,trk_innovation_matrix):
    ...
    if asso == 'm_dis':
        ...
        distance_matrix = compute_iou_distance(dets, tracks, asso,angle_thresh)
    elif asso == 'iou_2d':
        distance_matrix = compute_iou_distance(dets, tracks, asso,angle_thresh)
    elif asso == 'giou':
        distance_matrix = compute_iou_distance(dets, tracks, asso,angle_thresh)
...
def compute_iou_distance(dets, tracks, asso='iou',angle_thres=-1.0):
    ...
    for d, det in enumerate(dets):
        for t, trk in enumerate(tracks):
            ...
            if angle_thres>0:
                angle=mod_angle(det.o,trk.o)

                if abs(angle-trk.o) > angle_thres:
                   iou_matrix[d, t] = -10
