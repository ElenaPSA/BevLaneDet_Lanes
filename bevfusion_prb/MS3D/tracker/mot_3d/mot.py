...
class MOTModel:
    def __init__(self, configs):
        ...
        self.angle_thresh = configs['running']['angle_thres']
    ...
    def frame_mot(self, input_data: FrameData):
            ...
            if t not in unmatched_trks:
                ...
                if self.has_velo:
                    aux_info = {
                        ...
                        'ori_class_id': input_data.det_ori_types[d]
                        }
                else:
                ...
                Life Cycle Management: Two-stage association section (in paper aka. "self.redundancy" here)
                Early termination is the dominating cause of ID switches. For missing detections, paper
                proposes to replace low score detections with tracklets without updating motion model
                """
              
                result_bbox, update_mode, aux_info = self.redundancy.infer(trk, input_data, time_lag)
                aux_info = {'is_key_frame': input_data.aux_info['is_key_frame'],
                            'ori_class_id': trk.det_ori_type[-1]}
        ...
        for index in unmatched_dets:
            if self.has_velo:
                aux_info = {
                    ...
                    'is_key_frame': input_data.aux_info['is_key_frame'],
                    'ori_class_id': input_data.det_ori_types[index]}
            else:
                aux_info = {'is_key_frame': input_data.aux_info['is_key_frame'],
                            'ori_class_id': input_data.det_ori_types[index]}
            
            track = tracklet.Tracklet(self.configs, self.count, input_data.dets[index],
                                       input_data.det_types[index],input_data.det_ori_types[index],
        ...
        for trk in self.trackers:
            ...
            result.append((trk.get_state(), trk.id, state_string, trk.det_type,trk.get_detection()))
    ...
    def forward_step_trk(self, input_data: FrameData):
        ...
        for trk in self.trackers:
            trk_box = trk.predict(input_data.time_stamp, input_data.aux_info['is_key_frame'], input_data.delta_ego)
            trk_preds.append(trk_box)
        ...
        matched, unmatched_dets, unmatched_trks = associate_dets_to_tracks(dets, trk_preds, 
            self.match_type, self.asso,self.angle_thresh, self.asso_thres, trk_innovation_matrix)
