...
class NuScenesDataset(Custom3DDataset):
    ...
    def get_data_info(self, index: int) -> Dict[str, Any]:
        ...
        if  isinstance(info["ego2global_rotation"], np.ndarray):
        ...
        if 'postlidar_translation' in info.keys():
            data["postlidar_translation"]=info["postlidar_translation"]
            data["postlidar_rotation"]=Quaternion(info["postlidar_rotation"]).rotation_matrix
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            annos = []
            boxes = output_to_nusc_box(det,self.object_classes)
        ...
        mmcv.dump(nusc_submissions,'./results_nusc.json')
        return res_path
    ...
    def _evaluate_single(
        ...
        if len(self.object_classes)==10:
            ...
            from .EvalFunctions import evaluation, load_gt, load_prediction
            
            gt_boxes=load_gt(self.ann_file)
            pred_boxes,meta=load_prediction(result_path,self.eval_detection_configs.max_boxes_per_sample, DetectionBox,
                                                     verbose=False)
            metrics_summary=evaluation(gt_boxes,pred_boxes,self.eval_detection_configs,self.object_classes,meta,output_dir)
...
def output_to_nusc_box(detection,object_classes):
    ...
    from .EvalFunctions import class_range
    box_list = []
    for i in range(len(box3d)):
        ...
        if np.sqrt(np.sum(np.array(box.center[:2]) ** 2))<class_range[object_classes[labels[i]]]:
                box_list.append(box)
        box_list.append(box)
    return box_list
