...
class LoadPointsFromMultiSweeps:
    ...
    def __call__(self, results):
        ...
        if self.test_mode:
            ...
            use_static=np.random.random_sample()<0.2
        ...
        if self.pad_empty_sweeps and len(results["sweeps"]) == 0:
            ...
            if use_static:
                for idx in choices:
                    ...
                    points_sweep = points.clone()
                    sweep_ts = sweep["timestamp"] / 1e6
                    points_sweep.tensor[:, 4] = ts - sweep_ts
                ...
                for idx in choices:
                    ...
                    if len(points_sweep.shape)==2 and points_sweep.shape[1]< self.load_dim:
                        points_=np.zeros((points_sweep.shape[0],self.load_dim),dtype=points_sweep.dtype)
                        points_[:,0:points_sweep.shape[1]]=points_sweep[:,0:points_sweep.shape[1]]
                        points_sweep=points_
...
class LoadPointsFromFile:
    ...
    def __call__(self, results):
        ...
        if len(points.shape)>1 and points.shape[1]< self.load_dim:
            points_=np.zeros((points.shape[0],self.load_dim),dtype=points.dtype)
            points_[:,0:points.shape[1]]=points[:,0:points.shape[1]]
            points=points_
        ...
        if 'postlidar_translation' in results.keys():

            points[:, 0:3] = (
                points[:, 0:3] @ results["postlidar_rotation"].T
            )
            points[:, 0:3] += results["postlidar_translation"]
