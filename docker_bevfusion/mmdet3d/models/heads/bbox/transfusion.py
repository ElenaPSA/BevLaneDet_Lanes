...
class TransFusionHead(nn.Module):
    ...
    def forward_single(self, inputs, img_inputs, metas):
        ...
        if self.test_cfg["dataset"] == "nuScenes" and heatmap.size()[1]==10:
            local_max[
                :,
                8,
        ...
        if self.test_cfg["dataset"] == "nuScenes" and heatmap.size()[1]==6:
            local_max[
                :,
                5,
            ] = F.max_pool2d(heatmap[:, 5], kernel_size=1, stride=1, padding=0)
        if self.test_cfg["dataset"] == "Waymo":  # for Pedestrian & Cyclist in Waymo
            local_max[
                :,
                1,
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for idx_layer in range(self.num_decoder_layers if self.auxiliary else 1):
            ...
            # print(layer_reg_weights.shape)
            # print(code_weights.shape)
